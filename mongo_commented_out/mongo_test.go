...
// has been commented out. (it could easily be put back in if one needs to use with mongo).
...
func TestSuccessfulCloseMongoSession(t *testing.T) {
	...
	Convey("Timed out from safely closing mongo session", t, func() {
		Convey("with no context deadline", func() {
			...
			go func() {
				_ = slowQueryMongo(4 /*copiedSession*/)
			}()
			...
			err := Close(ctx, 5 /*copiedSession*/) // NOTE: the 'session.Close()' in the call to this Close() is not reached due to 'ctx' exit clause.
		...
		Convey("with context deadline", func() {
			//copiedSession := session.Copy()
			go func() {
				_ = slowQueryMongo(6 /*copiedSession*/)
			}()
			// Sleep for half a second for mongo query to begin
			time.Sleep(500 * time.Millisecond)
...
func cleanupTestData(ses int /*session *mgo.Session*/) error {
	defer func() {
...
func slowQueryMongo(ses int /*session *mgo.Session*/) error {
	defer func() {
...
func setUpTestData(ses int /*session *mgo.Session*/) error {
	defer func() {
...
func setupSession() (int /* *Mongo*/, error) {
	//	mongo := &Mongo{
